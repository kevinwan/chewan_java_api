...
public class MessageService {
    ...
    public static boolean sendMessage(String phone, String verifyCode, Integer sendTimes) {
        ...
        if (PropertiesUtil.getProperty("carplay.status.online", true)) {
            // 正式上线运行，采用创蓝的短信验证码
            success = sendByChuanglan(phone, verifyCode);
        } else {
    ...
    private static boolean sendByChuanglan(String phone, String verifyCode) {
        boolean sendResult = true;
        try {
            SendMessageByChuanglan.sendMessage(phone, verifyCode);
        } catch (Exception e) {
