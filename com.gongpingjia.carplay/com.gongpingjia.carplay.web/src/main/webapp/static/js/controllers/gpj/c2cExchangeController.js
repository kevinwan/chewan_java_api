...
/**
 * c2cExchangeController
 *
 * @constructor
 */
gpjApp.controller('c2cExchangeController', ['$scope', '$rootScope', '$http', '$modal', 'DTOptionsBuilder', 'DTColumnDefBuilder', '$compile', 'DTInstances',
    'gpjService', function ($scope, $rootScope, $http, $modal, DTOptionsBuilder, DTColumnDefBuilder, $compile, DTInstances, gpjService) {
        ...
        $scope.dtColumnDefs = [
            ...
            DTColumnDefBuilder.newColumnDef(3).withOption('data', 'city').notSortable().withOption('width', '6%'),
            ...
            DTColumnDefBuilder.newColumnDef(9).withOption('data', 'source').notSortable(),
            DTColumnDefBuilder.newColumnDef(10).withOption('data', 'process_status').notSortable(),
            DTColumnDefBuilder.newColumnDef(11).withOption('data', null).renderWith(actionsHtml).notSortable(),
            DTColumnDefBuilder.newColumnDef(12).withOption('data', 'publish_id').notVisible().notSortable()
        ];
        ...
        $scope.dtOptions = DTOptionsBuilder.newOptions().withBootstrap()
            .withOption('ajax', function (data, callback, settings) {
                ...
                $rootScope.loadingPromise = gpjService.getC2CExchanges({
                    draw: data.draw,
                    length: data.length,
                    start: data.start,
        ...
        $scope.viewItem = function (itemId) {
            ...
            gpjService.setC2CExchange($scope.records[itemId]);

            var modalInstance = $modal.open({
                templateUrl: 'views/gpj/c2c_exchange_modal.html',
                controller: 'c2cExchangeModalController'
            });
