...
/**
 * noDealerWantCarsController
 *
 * @constructor
 */
gpjApp.controller('noDealerWantCarsController', ['$scope', '$rootScope', '$http', '$modal', 'DTOptionsBuilder', 'DTColumnDefBuilder', '$compile', 'DTInstances',
    'gpjService', function ($scope, $rootScope, $http, $modal, DTOptionsBuilder, DTColumnDefBuilder, $compile, DTInstances, gpjService) {
        ...
        $scope.dtColumnDefs = [
            ...
            DTColumnDefBuilder.newColumnDef(1).withOption('data', 'contact').notSortable(),
            ...
            DTColumnDefBuilder.newColumnDef(9).withOption('data', 'notify_count').notSortable(),
        ...
        $scope.changeProvince = function (provinceOption) {
            ...
            if (provinceOption && provinceOption.cities) {
                ...
                if ($scope.cityOptions.length === 1)
                    $scope.criteria.city = $scope.cityOptions[0];
            }
        };
        ...
        $scope.dtOptions = DTOptionsBuilder.newOptions().withBootstrap()
            .withOption('ajax', function (data, callback, settings) {
                ...
                $rootScope.loadingPromise = gpjService.getNoDealerWantCars({
                    draw: data.draw,
                    length: data.length,
                    start: data.start,
        ...
        $scope.viewItem = function (itemId) {
            ...
            gpjService.setNoDealerTrade($scope.records[itemId]);

            var modalInstance = $modal.open({
                templateUrl: 'views/gpj/nodealer_want_cars_modal.html',
                controller: 'noDealerTradeModalController'
            });
